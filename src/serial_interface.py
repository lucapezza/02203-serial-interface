        ...
        if not(serial_available):
            ...
            if testSerial():
                ...
                for i in range(0, 288):
                    ser.write(download_image[i * 352:(i + 1) * 352])
                    download_progress = int(100 * (i / (288 - 1)))
...
def updateEnableDisable():
    ...
    # Button: Upload show
    if serial_free and upload_image_exists:
...
app.setEntry("entry_d1", "Select PGM image to download.", callFunction=False)
...
app.addNamedButton("Show image", "button_d2", showDownloadImage, row, 2)
...
app.addNamedButton("Show image", "button_u2", showUploadImage, row, 2)
